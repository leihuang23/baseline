...
.PHONY: dev test lint typecheck migrate fmt eval demo docs-check task-status task-next task-current task-current-watch task-finish task-finish-commit task-loop-one task-loop-one-codex task-loop-one-commit task-loop-one-commit-codex task-loop-p0-cluster task-loop-p0-cluster-codex
